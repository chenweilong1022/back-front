<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column align="center" label="日注册数">
        <template slot-scope="scope">
          {{ scope.row.dayReg }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="日活跃数">
        <template slot-scope="scope">
          {{ scope.row.dayActive }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="日登录次数">
        <template slot-scope="scope">
          {{ scope.row.dayLogin }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="日最高在线">
        <template slot-scope="scope">
          {{ scope.row.dayMaxOnline }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="日平均在线">
        <template slot-scope="scope">
          {{ scope.row.dayAvgOnline }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="日新增充值玩家">
        <template slot-scope="scope">
          {{ scope.row.dayNewRecharge }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="日总充值玩家">
        <template slot-scope="scope">
          {{ scope.row.dayTotalRecharge }}
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    query() {
      ...
      this.$store.dispatch('ListDailyPlayersCountByQry', this.filterForm).then(resp => {
        this.list = resp.data
