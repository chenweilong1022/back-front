<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column align="center" label="用户帐号">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="周期佣金">
        <template slot-scope="scope">
          每万元 {{ scope.row.ratio ? scope.row.ratio : 0 }} 元
        </template>
      </el-table-column>
      <el-table-column align="center" label="返佣总金额">
        <template slot-scope="scope">
          {{ scope.row.rebate | moneyFilter }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    query() {
      ...
      this.$store.dispatch('ListAgentRealtimeByQry', { ...this.filterForm, superId: this.superId }).then(resp => {
        this.list = resp.data
    ...
    handleClick(superId) {
      this.$router.replace({ path: '/spread/underAgentRealtime', query: { superId }})
    },
