<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column align="center" label="日订单钻石汇总">
        <template slot-scope="scope">
          {{ scope.row.totalOrderDiamond }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="日提成钻石汇总">
        <template slot-scope="scope">
...
<script>
export default {
  name: 'DailyCountSpreadDiamondPage',
  filters: {
    moneyFilter(money) {
      if (money) {
        return money
  ...
  methods: {
    query() {
      ...
      this.$store.dispatch('ListDailyCountSpreadDiamondRecordByQry', this.filterForm).then(resp => {
        this.list = resp.data
