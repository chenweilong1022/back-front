<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column align="center" label="日志路径">
        <template slot-scope="scope">
          {{ scope.row.logFile }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="监控关键字">
        <template slot-scope="scope">
          {{ scope.row.monitorKeyword }}
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogNewOrEditVisible" :title="newFlag | dialogTitle" width="600px">
      <el-form ref="dataForm" :model="data" :rules="dataRule" label-width="100px">
        <el-form-item label="日志路径" prop="logFile">
          <el-input v-model="data.logFile" placeholder="日志路径"/>
        </el-form-item>
        <el-form-item label="监控关键字" prop="monitorKeyword">
          <el-input v-model="data.monitorKeyword" placeholder="监控关键字"/>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :loading="saveBtnLoading" type="primary" @click="savetblmonitor"><svg-icon icon-class="save" style="margin-right: 5px;"/>保存</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        monitorKeyword: [
          { required: true, message: '监控关键字不能为空', trigger: 'blur' }
        ],
        createTime: [
  ...
  methods: {
    query() {
      ...
      this.$store.dispatch('Listtblmonitor', this.filterForm ).then(resp => {
        this.list = resp.data
      ...
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          this.$confirm('是否确定保存?', '提示', {
            ...
            this.$store.dispatch(this.newFlag ? 'Savetblmonitor' : 'Updatetblmonitor', this.data).then(resp => {
    ...
    removeData(data) {
      this.$confirm('是否确定删除?', '提示', {
        ...
        this.$store.dispatch('Deletetblmonitor', data.id).then(resp => {
          this.$message({
            message: resp.msg,
            type: 'success'
          })
